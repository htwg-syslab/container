...
5. Zugriff auf den Container so dass die DISPLAY Variable entspr. gesetzt wird: `ssh -Y bsyslab` bzw. `ssh -Y esyslab`.
...
8. Anschließend sollte xhost korrekt funktionieren, und Sie können sich über den Befehl `ssh -Y -o ForwardX11=yes -o ForwardX11Trusted=yes bsyslab` (bzw. `esyslab`) mit dem Container verbinden.
...
Unter Linux läuft X11 nativ, daher sind keine weiteren Schritte erforderlich. Zum korrekten Setzen der DISPLAY-Variable den SSH-Zugriff via `ssh -Y bsyslab` (bzw. `ssh -Y esyslab`) starten.
