...
## Installation unter Ubuntu (Docker Image)

Um **bat** unter Ubuntu zu installieren und es in Ihrer `.bashrc` zu konfigurieren, folgen Sie diesen Schritten:

### Schritt 1: Installation von Bat unter Ubuntu
...
2. Installieren Sie das **bat** Paket:
   ...
   **Hinweis**: Unter Ubuntu wird **bat** möglicherweise als `batcat` installiert, um Namenskonflikte mit anderen Programmen zu vermeiden.

### Schritt 2: Alias in der `.bashrc` hinzufügen

Da **bat** unter Ubuntu möglicherweise als `batcat` installiert wird, können Sie einen Alias erstellen, damit der Befehl `bat` funktioniert, wie erwartet.
